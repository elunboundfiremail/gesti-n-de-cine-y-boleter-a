...
export default function ClientPanel({
  ...
  const handleBuyTicketSubmit = (e: React.FormEvent) => {
    ...
    // Simulate online QR scanning payment duration
    setTimeout(() => {
      const room = rooms.find(r => r.id === selectedShow.roomId);
      const isPromoActive = getShowOccupancy(selectedShow.id).percent < 30;
      
      // Calculate 2x1 discount
      // Buy 2 pay 1. Ex: if 2, pay for 1. If 3, pay for 2. If 1, pay for 1.
      let pricePaid = 0;
      if (isPromoActive) {
        const pairs = Math.floor(quant / 2);
        const remainder = quant % 2;
        pricePaid = (pairs + remainder) * selectedShow.price;
      } else {
        pricePaid = quant * selectedShow.price;
      }

      // Generate assigned seat sequences based on seating mode choice
      // Let's assign seats or sequence labels
      const newBookings: TicketRef[] = [];
      const formattedTimestamp = new Date().toISOString();
      const baseSeatIndex = sold + 1;

      const seatArray: string[] = [];
      if (selectedMovie.allowSeatSelection) {
        seatArray.push(...selectedSeats);
      } else {
        for (let i = 0; i < quant; i++) {
          const seatNo = baseSeatIndex + i;
          // Simple automatic alphabetical listing
          const rowLetter = String.fromCharCode(65 + Math.floor((seatNo - 1) / 10)); // e.g. A, B...
          const seatNumInRow = ((seatNo - 1) % 10) + 1;
          
          // Simulates option of automatic seating assignment vs order of arrival
          // We will include both details on the ticket!
          seatArray.push(`Fila ${rowLetter}-Asiento ${seatNumInRow}`);
        }
      }

      const orderString = `Entrada General # ${baseSeatIndex} al ${baseSeatIndex + quant - 1} (Por Orden de Llegada / General Admission)`;
      ...
      const newTicket: TicketRef = {
        ...
        pricePaid: pricePaid,
        nit: clientNit || 'C/F', // C/F is standard "Consumidor Final" in Latin America
  ...
  return (
    <div className="bg-slate-50 min-h-screen text-slate-800 font-sans pb-12" id="client-view">
      ...
      <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white py-12 px-6 rounded-b-[2rem] shadow-xl text-center relative overflow-hidden" id="client-jumbo">
        ...
        <div className="max-w-4xl mx-auto relative z-10 space-y-3">
          ...
          <p className="text-[10px] text-slate-400 mt-2">
            ⚠️ Regulaciones: Compra habilitada para máximo 24 horas de anticipación (1 día antes).
          </p>
        </div>
      </div>
      ...
      {/* MY TICKETS HISTORIC DASHBOARD FOR REGISTERED CUSTOMERS */}
      {currentUser && (
        <div className="max-w-6xl mx-auto px-4 mt-12 pt-8 border-t border-slate-200" id="personal-tickets-dashboard">
          ...
          {(() => {
            // Filter tickets where NIT matches, or name matches (just in case they changed during checkout)
              ...
              return (
                <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center text-slate-500 max-w-md mx-auto">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Cualquier boleto que compres con tu NIT ({currentUser.nit || 'C/F'}) o Nombre se mostrará automáticamente en esta sección.
                  </p>
                </div>
              );
            }
