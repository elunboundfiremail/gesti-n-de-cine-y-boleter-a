          ...
          <div className="hidden md:flex bg-slate-950 px-4 py-2 rounded-xl border border-slate-800 items-center gap-2" id="identity-header-badge">
            ...
            <span className="text-xs font-bold text-slate-300">
              ...
              <span className="text-white uppercase font-extrabold text-[12px] tracking-wide">
                {currentUser === null && 'Cartelera Pública (Invitado)'}
      ...
      <footer className="bg-white border-t border-slate-200 py-6 px-4 shrink-0 font-sans text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="font-medium text-slate-400">
            © 2026 Sistema Integrado de Salas y Boletería • Todo el Físico sin Elección de Asientos Gráfica Manual • Boliviana de Cine.
          </p>
          <div className="flex gap-4 items-center flex-wrap justify-center sm:justify-start font-mono text-[10px]">
            <span>Estable: Grande A/B, Media, Pequeña</span>
            <span>Triggers: &lt;30% (Promo 2x1)</span>
            <span className={`px-2 py-0.5 rounded font-sans font-semibold border ${
              USE_REAL_BACKEND 
                ? 'bg-emerald-50 text-emerald-700 border-emerald-100' 
                : 'bg-indigo-50 text-indigo-700 border-indigo-150'
            }`}>
              {USE_REAL_BACKEND ? `📡 REST API: ${API_BASE_URL}` : '💾 Local Storage Backend (Producción Listo)'}
            </span>
          </div>
        </div>
      </footer>
